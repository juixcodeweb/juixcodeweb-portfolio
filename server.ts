...
// POST Contact Form / Project Inquiry
app.post('/api/contact', async (req, res) => {
  try {
    ...
    if (!fullName || !emailAddress || !phoneNumber || !projectMessage) {
      return res.status(400).json({ 
        success: false, 
        message: 'Missing required fields: fullName, emailAddress, phoneNumber, and projectMessage.' 
      });
    }
    ...
    let emailsSent = false;
    let emailStatusMsg = 'Contact inquiry registered.';
    ...
    if (smtpPass) {
        ...
        html: `
          <div style="font-family: Arial, sans-serif; padding: 20px; color: #111827; background-color: #f9fafb;">
            <div style="max-width: 600px; margin: 0 auto; background: #ffffff; border-radius: 12px; padding: 24px; border: 1px solid #e5e7eb;">
              ...
              <p style="font-size: 14px; color: #4b5563;">You have received a new project inquiry from your website contact form:</p>
      ...
      try {
        ...
        console.error('SMTP Email Error:', err?.message || err);
        emailStatusMsg = `Inquiry saved, but email error: ${err?.message || 'Check SMTP configuration'}`;
      }
    }
    ...
    return res.status(500).json({
      success: false,
      message: 'Failed to process project message.',
      error: error?.message || 'Server error'
    });
  }
});
